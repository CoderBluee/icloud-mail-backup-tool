...
# Description: Automated end-to-end pipeline to compile Prism iCloud Mail Backup Tool Tool,
...
param (
    ...
    [switch]$AppImageOnly      # Pass -AppImageOnly to stop after creating Prism-iCloud-Mail-Backup-Tool.exe
)
...
$AppFolder = Join-Path $AppImageDir "Prism-iCloud-Mail-Backup-Tool"
...
Write-Host "  Prism iCloud Mail Backup Tool Tool - Full EXE & Installer Pipeline" -ForegroundColor Cyan
...
$JpackageArgs = @(
    "--type", "app-image",
    "--name", "Prism-iCloud-Mail-Backup-Tool",
...
$ExePath = Join-Path $AppFolder "Prism-iCloud-Mail-Backup-Tool.exe"
if (-not (Test-Path $ExePath)) {
    Write-Error "Prism-iCloud-Mail-Backup-Tool.exe was not created at expected location: $ExePath"
}
...
& $AiPath /edit $AipFile /SetProperty ProductName="Prism iCloud Mail Backup Tool Tool"
...
& $AiPath /edit $AipFile /NewShortcut -name "Prism iCloud Mail Backup Tool Tool" -dir DesktopFolder -target "[APPDIR]Prism-iCloud-Mail-Backup-Tool\Prism-iCloud-Mail-Backup-Tool.exe" -icon "$LogoIcon"
& $AiPath /edit $AipFile /NewShortcut -name "Prism iCloud Mail Backup Tool Tool" -dir SHORTCUTDIR -target "[APPDIR]Prism-iCloud-Mail-Backup-Tool\Prism-iCloud-Mail-Backup-Tool.exe" -icon "$LogoIcon"
...
& $AiPath /edit $AipFile /SetPackageName "Prism-iCloud-Mail-Backup-Tool-Setup.exe" -buildname DefaultBuild
...
# 10. Sync to Drive
$driveJarDir = "I:\My Drive\Akas\Jars\iCloud Mail Backup Tool"
if (-not (Test-Path $driveJarDir)) { New-Item -ItemType Directory -Path $driveJarDir -Force | Out-Null }
Copy-Item $ProtectedJar.FullName (Join-Path $driveJarDir "Prism-iCloud-Mail-Backup-Tool.jar") -Force
...
Copy-Item $AipFile (Join-Path $destAdvance "Prism-iCloud-Mail-Backup-Tool.aip") -Force
...
Write-Host "     -> Prism-iCloud-Mail-Backup-Tool-Setup.exe" -ForegroundColor White
Write-Host "  3. Standalone Protected JAR: $driveJarDir" -ForegroundColor White
Write-Host "     -> Prism-iCloud-Mail-Backup-Tool.jar" -ForegroundColor White
Write-Host "========================================================" -ForegroundColor Green
